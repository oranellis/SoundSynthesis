...
std::vector<int8_t> MakeSounds() {
	...
	for (uint64_t sample_iterator = 0; sample_iterator < total_sample_count; sample_iterator++) {
		sound_sample = 16384 * std::sin(sin_counter);
...
int main(int argc, char** argv) {
	std::ofstream wav_output_file("example.wav", std::ios::out | std::ios::binary);
	if(!wav_output_file) {
		std::cout << "Cannot open file!" << std::endl;
		return 1;
	}
	std::vector<int8_t> sound_data;
	sound_data = MakeSounds();
